...
from lab3.layer.layer3 import ContentLossLayer,StyleLossLayer
...
class VGG19(object):
    ...
    def load_image(self, image_dir, image_height, image_width):
        ...
        image_shape = self.input_image.shape
        self.input_image = resize(self.input_image, [image_height, image_width,3], anti_aliasing=True, preserve_range=True)
        self.input_image = np.array(self.input_image).astype(np.float32)
        self.input_image -= self.image_mean
        self.input_image = np.reshape(self.input_image, [1] + list(self.input_image.shape))
        # input dim [N, channel, height, width]
    ...
    #     input_image = resize(input_image, image_shape, anti_aliasing=True, preserve_range=True)
    #     imageio.imwrite(image_dir, input_image)

    def save_image(self, input_image, image_shape, image_dir):
        # Adjust the shape of the input image
        input_image = np.transpose(input_image, [0, 2, 3, 1])  # [N, H, W, C] -> [N, W, H, C]
        input_image = input_image[0] + self.image_mean  # Add image mean
        input_image = np.clip(input_image, 0, 255)  # Clip values to [0, 255]
        input_image = resize(input_image, image_shape, anti_aliasing=True, preserve_range=True)
...
if __name__ == '__main__':
    ...
    content_image, content_shape = vgg.load_image('./lab3/02/weinisi.jpg', IMAGE_HEIGHT, IMAGE_WIDTH)
    style_image, _ = vgg.load_image('./lab3/02/xingyueye.jpg', IMAGE_HEIGHT, IMAGE_WIDTH)
    ...
    for step in range(TRAIN_STEP):
        ...
        if step % 20 == 0:
            # print('Step %d, loss = %f' % (step, total_loss), content_loss, style_loss)
            vgg.save_image(transfer_image, content_shape, './lab3/02/output_' + str(step) + '.jpg')
