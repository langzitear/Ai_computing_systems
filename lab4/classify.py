...
if __name__ == '__main__':
    IMAGE_PATH = './image/strawberries.jpg'
    PTH_PATH = './pth/imagenet-vgg-verydeep-19.pth'
    input_image = load_image(IMAGE_PATH)
    #TODO: 生成VGG19网络模型并保存在net中
    net = vgg19()
    net.load_state_dict(torch.load(PTH_PATH,weights_only=False))
    ...
    # print(f'Predicted class label: {response[predicted.item()]}')
    #
    ...
    with open('./labels/imagenet_classes.txt') as f:
        ...
        print(indices.shape)
    print("Classification result: id = %s, prob = %f " % (classes[indices[0][0]], output[0][indices[0][0]].item()))
